// Function to create browser notifications for tasks
...
// Function to generate recurring tasks based on interval
function generateRecurringTasks() {
    ...
    formData.forEach(task => {
        ...
        // If the days difference is divisible by the interval, add the task to events array
        if (daysDifference >= 0 && daysDifference % interval === 0) {
            const newTask = {
                event_title: task.task,
                event_category: task.category,
                event_date: formatDate(today), // Use today's date as the event date
                event_interval: task.interval,
                event_description: task.description
            };

            // Add the new task to the events array
            myApp.events.push(newTask);
        }
    });
};

document.addEventListener('DOMContentLoaded', function() {
    // Example usage of schedule notification:
    ...
    myApp.initDate();
    myApp.getNoOfDays();
    generateRecurringTasks(); // Generate recurring tasks based on interval
});
