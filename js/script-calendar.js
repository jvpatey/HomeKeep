...
function convertToMMDDYYYY(dateString) {
    ...
    if (isNaN(date.getTime())) {
        ...
        // If the date is valid, proceed with formatting
...
// function to save task data from add task form
...
// Mapping interval value to display text
...
// function to display tasks in calendar
async function displayTasks(user, year, month) {
    ...
    try {
        ...
        // get stored task data from firestore
        querySnapshot.forEach((doc) => {
            ...
            // get next occurance of task based on start date and interval
            ...
            while (nextOccurrenceDate <= lastDayOfMonth) {
                if (nextOccurrenceDate >= firstDayOfMonth && nextOccurrenceDate <= lastDayOfMonth) {
                    ...
                    // create event container for each task
                    if (cell) {
...
// event listener for current month button
...
// Function to fetch modal content from modals.html
function loadModals() {
...
// initialize functions and event listener for edit submit btn
...
// function to show task details when task is clicked in calendar
function showTaskDetailsModal(taskData) {
    ...
    // fetch task data to populate modal
    const taskDetailsModal = document.getElementById('taskDetailsModal');
...
// Event listener to show task details modal when event block is clicked in calendar
...
/* ----- Handling help request pop up form ----- */
